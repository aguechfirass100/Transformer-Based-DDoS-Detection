...
def create_inference_folder():
    ...
    """
    base_path = Path(
        "C:/Users/AGFirass/Documents/GitHub/Transformer-Based-DDoS-Detection/models/model_multi_attention/inference")
...
# === MODIFIED ===: Added leakage_test_config parameter
def evaluate_model(model, test_loader, device='cuda' if torch.cuda.is_available() else 'cpu', leakage_test_config=None):
    """
    ...
    # === NEW ===: Get leakage test info
    target_group = None
    target_index = -1
    if leakage_test_config and leakage_test_config.get('group') and leakage_test_config.get('index', -1) != -1:
        target_group = leakage_test_config['group']
        target_index = leakage_test_config['index']
        print(f"--- Leakage Test Active: Modifying group '{target_group}', index {target_index} ---")
    # === END NEW ===

    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):  # === NEW: added batch_idx for print warning once
            # Updated to handle the new data structure: features dict, protocol_ids, labels
            if len(batch) == 3:
                features, protocol_ids, labels = batch

                # === MODIFICATION TO SET FEATURE TO CONSTANT (UPDATED) ===
                if target_group and target_group in features:
                    try:
                        # features[target_group] is shape (batch_size, num_features_in_group)
                        # We modify the column at target_index for all items in the batch
                        constant_value = 0.0
                        features[target_group][:, target_index] = constant_value
                    except IndexError:
                        if batch_idx == 0:  # Print warning only once
                            print(f"Error: Index {target_index} out of bounds for group '{target_group}'.")
                    except Exception as e:
                        if batch_idx == 0:  # Print warning only once
                            print(f"Error modifying feature: {e}")
                elif target_group and batch_idx == 0:  # Print warning only once
                    # This check is now for the *group*
                    print(f"Warning: Feature group '{target_group}' not found in batch features.")
                # === END OF MODIFICATION ===
...
def main():
    ...
    try:
        ...
        checkpoint_path = r"C:\Users\AGFirass\Documents\GitHub\Transformer-Based-DDoS-Detection\models\model_multi_attention\lightning_logs\checkpoints\best-checkpoint-08-0.9999.ckpt"
    ...
    # === NEW: Find timestamp index ===
    # Use the exact key from your config.yaml
    timestamp_key = 'Timestamp'
    target_group = None
    target_index = -1

    # Use the feature_groups from the loaded config
    if 'feature_groups' in config:
        for group, features_list in config['feature_groups'].items():
            if features_list and timestamp_key in features_list:
                target_group = group
                target_index = features_list.index(timestamp_key)
                break

    leakage_config = None
    if target_group:
        print(f"Found '{timestamp_key}' in group '{target_group}' at index {target_index}.")
        leakage_config = {'group': target_group, 'index': target_index}
    else:
        print(f"Warning: '{timestamp_key}' not found in config feature_groups. Leakage test inactive.")
    # === END NEW ===
    ...
    # === MODIFIED ===: Pass leakage_config to evaluate_model
    all_preds, all_labels, all_probs = evaluate_model(
        model,
        test_loader,
        device,
        leakage_test_config=leakage_config
    )
    ...
    for i, class_name in enumerate(class_names):
        class_key = str(unique_classes[i])  # Use the actual class index
        ...
        elif str(i) in report:  # Fallback for simple 0, 1, 2... keys
    ...
    with open(results_file, 'w') as f:
        ...
        f.write(f"\nClassification Report:\n")
        f.write("=" * 80 + "\n")
        # === MODIFIED ===: Use unique_classes as labels for classification_report
        f.write(classification_report(all_labels, all_preds, target_names=class_names, labels=unique_classes,
                                      zero_division=0))
        f.write(f"\n\nClass Names Used:\n")
        for i, name in enumerate(class_names):
            f.write(f"Class {unique_classes[i]}: {name}\n")  # Map actual index to name
...
#                 feature_key = 'timestamp'  # Make sure this is the exact key name
#                 if feature_key in features:
#                     # Create a tensor full of a constant (e.g., 0.0)
#                     # with the same shape and data type as the original timestamp tensor.
#                     constant_value = 0.0
#                     features[feature_key] = torch.full_like(
#                         features[feature_key],
#                         constant_value
#                     )
#                 else:
#                     print(f"Warning: Feature key '{feature_key}' not found in batch features.")
#                 # === END OF MODIFICATION ===
#
#                 # Move features dict to device
#                 features_device = {}
