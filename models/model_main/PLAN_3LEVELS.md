...
### 2. Implicit Transformer Branches (Per Flow Processing)

#### A. Packet-Level Implicit Transformer
```
...
└── Output: implicit_packet_repr[144]
```

#### B. Flow-Level Implicit Transformer  
```
Input: flow_features[86] per flow (ungrouped)
│
├── Feature Embedding: 86 features → sequence of 86 tokens
├── Linear Projection: each feature → 64 dims
├── Positional Encoding: Add feature position info
│   └── Embedded Flow Features: [86, 64]
│
...
│   └── Residual Connections + LayerNorm
│
├── Global Average Pooling: [86, 64] → [64]
└── Output: implicit_flow_repr[64]
```
...
```
Per Flow:
├── implicit_packet_repr[144]      # From packet-level implicit transformer
├── implicit_flow_repr[64]         # From flow-level implicit transformer  
...
Concatenation: [144 + 64 + 64 + 64 + 64 + 64] = [464]
│
├── Linear Projection: 464 → 256
...
### 1. **Dual Implicit + Explicit Fusion**
- **Packet-Level Implicit**: Learns general packet sequence patterns
- **Flow-Level Implicit**: Discovers hidden feature interactions automatically
- **Explicit Heads**: Focus on domain-specific attack signatures
- **Combination**: Captures learned, discovered, AND interpretable features
